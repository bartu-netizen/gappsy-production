// Verifies the new-batch-pairs-remaining.json list: dedupes, checks both tools exist
...
const rawPairs = JSON.parse(readFileSync(join(__dirname, 'new-batch-pairs-remaining.json'), 'utf-8'));
...
async function main() {
  ...
  writeFileSync(join(__dirname, 'verified-pairs-remaining.json'), JSON.stringify(valid, null, 2), 'utf-8');
  console.log(`\nWrote verified-pairs-remaining.json (${valid.length} pairs)`);
}
