// Pure functions for /roundup page prerendering (N-tool comparisons, 3+).
// Mirrors comparison-seo-generator.js's structure and reuses the same
// generic tag/JSON-LD injection plumbing from tool-seo-generator.js —
// only the comparison-specific parts here are generalized from exactly
// 2 tools to an arbitrary list, matching what RoundupDetailPage.tsx (and
// its src/components/roundup/* sections) build client-side.
...
export function generateRoundupSEOData({ roundup, tools, roundupContent }) {
  const title = `${roundup.title} | Gappsy`;
  const description = roundupContent?.verdict
    ? truncate(roundupContent.verdict, META_DESCRIPTION_MAX)
    : `Compare ${tools.map((t) => t.name).join(', ')} on pricing, features, and who each one is built for.`;
  const canonical = `${DOMAIN}/roundup/${roundup.slug}/`;
  const ogImage = tools.find((t) => isSafeHttpUrl(t.logo))?.logo || `${DOMAIN}/og/default-og-image.svg`;
  return { title, description, canonical, ogImage, slug: roundup.slug };
}

const GAPPSY_ORG = { '@type': 'Organization', name: 'Gappsy', url: DOMAIN };

export function generateRoundupJSONLD({ roundup, tools, roundupContent, seoData, createdAt, updatedAt }) {
  ...
  graph.push({
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Roundups', item: `${DOMAIN}/roundup/` },
      { '@type': 'ListItem', position: 3, name: roundup.title, item: canonical },
    ],
  });
...
export function generateRoundupStaticBodyHTML({ roundup, tools, roundupContent, seoData }) {
  const toolCardsHTML = `
    <div style="display:flex;flex-wrap:wrap;gap:1rem;margin:1.5rem 0;">
      ${tools
        ...
        ${roundupContent?.bestFor?.[t.slug] ? `<p style="color:#374151;font-size:0.8rem;margin-top:0.5rem;"><strong>Best for:</strong> ${escapeHtml(roundupContent.bestFor[t.slug])}</p>` : ''}
      </div>`
        )
        .join('')}
    </div>`;

  const highlightsHTML = roundupContent?.highlights?.length
  ...
  const featureMatrixHTML = roundupContent?.featureMatrix?.length
  ...
  const faqHTML = roundupContent?.faqs?.length
  ...
  return `
    ...
    <div style="max-width:960px;margin:0 auto;padding:20px;font-family:system-ui,-apple-system,sans-serif;">
      <nav style="font-size:0.85rem;color:#6B7280;margin-bottom:1rem;">
        ...
        <a href="/roundup/" style="color:#6B7280;">Roundups</a> ·
        ${escapeHtml(roundup.title)}
      </nav>
      <h1 style="font-size:2.25rem;font-weight:800;margin-bottom:1rem;color:#111827;">${escapeHtml(roundup.title)}</h1>
