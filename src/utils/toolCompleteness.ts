// Deterministic tool-completeness scoring — frontend (Vite/browser) side.
// MUST stay in sync with supabase/functions/_shared/toolCompleteness.ts
// (the edge-function mirror). Two copies, not a shared import, because
// supabase/functions (Deno) and src/ (Vite/browser) are different
// runtimes/toolchains — same pattern already used for
// generateToolDescription between scripts/tool-seo-generator.js and
// src/pages/ToolDetailPage.tsx.
//
// Used for live completeness feedback in the tool editor and the Tools
// list column. The authoritative publish gate is server-side, in
// admin-tools/index.ts's validatePublishRequirements — this is UX only.
//
