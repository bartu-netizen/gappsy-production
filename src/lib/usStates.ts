...
export function findState(query: string): USState | null {
  const normalized = query.trim().toLowerCase();

  if (!normalized) return null;

  // Try exact abbreviation match first
  const abbrMatch = US_STATES.find(state => state.abbr.toLowerCase() === normalized);
  if (abbrMatch) return abbrMatch;

  // Try exact name match
  const nameMatch = US_STATES.find(state => state.name.toLowerCase() === normalized);
  if (nameMatch) return nameMatch;

  // Try slug match
  const slugMatch = US_STATES.find(state => state.slug === normalized);
...
export function searchStates(query: string): USState[] {
  ...
  return US_STATES.filter(state =>
    ...
    state.slug.includes(normalized)
