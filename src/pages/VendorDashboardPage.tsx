...
export default function VendorDashboardPage() {
  ...
  if (authLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#f7f8fa]"><Loader2 className="w-6 h-6 text-indigo-500 animate-spin" /></div>;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f7f8fa] px-4">
        <div className="text-center">
          ...
          <Link to="/login" className="inline-flex items-center gap-1.5 bg-gradient-to-br from-[#4F46E5] to-[#7C3AED] text-white px-5 py-2.5 rounded-xl font-semibold text-sm">Sign in</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {loading && (
          <div className="flex items-center justify-center py-24"><Loader2 className="w-6 h-6 text-indigo-500 animate-spin" /></div>
        )}
                ...
                {TABS.map(({ key, label, icon: Icon }) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-colors text-left ${
                      tab === key ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:bg-white hover:text-[#0B1221]'
                    }`}
                  >
      ...
      <Card>
        <div className="flex items-start gap-4">
          {tool.logo ? (
            ...
            <div className="w-14 h-14 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-400 font-bold text-xl shrink-0">{tool.name.charAt(0)}</div>
          )}
          <div className="min-w-0 flex-1">
            ...
            <Link to={`/tools/${tool.slug}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[13px] font-medium text-indigo-600 hover:text-indigo-700 mt-2.5">
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full h-10 rounded-xl border border-slate-200 px-3.5 text-sm text-[#0B1221] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition-shadow"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <textarea
        ...
        onChange={(e) => onChange(e.target.value)}
        rows={rows}
        className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-[#0B1221] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition-shadow resize-y"
      />
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center gap-1.5 bg-gradient-to-br from-[#4F46E5] to-[#7C3AED] hover:from-[#4338CA] hover:to-[#6D28D9] text-white px-5 py-2.5 rounded-xl font-semibold text-sm transition-all disabled:opacity-60"
        >
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-4">
        ...
        <button type="button" onClick={add} className="inline-flex items-center gap-1 text-[13px] font-medium text-indigo-600 hover:text-indigo-700"><Plus className="w-3.5 h-3.5" /> Add</button>
      </div>
        ...
        {items.map((it, i) => (
          <div key={i} className="flex items-start gap-2 border border-slate-100 rounded-xl p-3">
            <div className="flex-1 min-w-0 space-y-2">
              <input value={it.title} onChange={(e) => update(i, 'title', e.target.value)} placeholder="Feature title" className="w-full h-9 rounded-lg border border-slate-200 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300" />
              <textarea value={it.description} onChange={(e) => update(i, 'description', e.target.value)} placeholder="Short description (optional)" rows={2} className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 resize-y" />
            </div>
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-4">
        ...
        <button type="button" onClick={() => setValues((prev) => [...prev, ''])} className="inline-flex items-center gap-1 text-[13px] font-medium text-indigo-600 hover:text-indigo-700"><Plus className="w-3.5 h-3.5" /> Add</button>
      </div>
      <div className="space-y-2">
        {values.length === 0 && <p className="text-[13px] text-slate-400">Nothing added yet.</p>}
        {values.map((v, i) => (
          <div key={i} className="flex items-center gap-2">
            <input value={v} onChange={(e) => update(i, e.target.value)} placeholder={placeholder} className="flex-1 h-9 rounded-lg border border-slate-200 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300" />
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-4">
        ...
        <button type="button" onClick={() => setItems((prev) => [...prev, { question: '', answer: '' }])} className="inline-flex items-center gap-1 text-[13px] font-medium text-indigo-600 hover:text-indigo-700"><Plus className="w-3.5 h-3.5" /> Add</button>
      </div>
      <div className="space-y-3">
        ...
        {items.map((it, i) => (
          <div key={i} className="flex items-start gap-2 border border-slate-100 rounded-xl p-3">
            <div className="flex-1 min-w-0 space-y-2">
              <input value={it.question} onChange={(e) => update(i, 'question', e.target.value)} placeholder="Question" className="w-full h-9 rounded-lg border border-slate-200 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300" />
              <textarea value={it.answer} onChange={(e) => update(i, 'answer', e.target.value)} placeholder="Answer" rows={2} className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 resize-y" />
            </div>
      ...
      {!isPending && (
        <div className="mt-4 pt-4 border-t border-slate-100">
          {review.vendor_response && (
            <div className="bg-indigo-50/60 rounded-xl p-3 mb-3">
              <p className="text-[11px] font-semibold text-indigo-600 mb-1">Your response</p>
          ...
          <div className="flex items-start gap-2">
            <textarea
              ...
              rows={2}
              className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 resize-y"
            />
            <button
              type="button"
  ...
  return (
    <Card>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center"><CreditCard className="w-5 h-5 text-indigo-500" /></div>
        <div>
