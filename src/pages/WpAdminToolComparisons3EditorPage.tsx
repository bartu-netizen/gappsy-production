...
interface GroupComparisonFormData {
  title: string;
  verdict: string;
  video_url: string;
...
interface GroupComparisonDetailResponse {
  ok: boolean;
  data: {
    id: string;
    slug: string;
...
const EMPTY_FORM: GroupComparisonFormData = {
  title: '',
  verdict: '',
  video_url: '',
  status: 'draft',
...
export default function WpAdminToolComparisons3EditorPage() {
  const { id } = useParams<{ id?: string }>();
  const navigate = useNavigate();
  const isEditMode = !!id;

  const { data: toolsData, isLoading: toolsLoading } = useAdminFetch<ToolsListResponse>('admin-tools');
  const { data, isLoading, isError, error } = useAdminFetch<GroupComparisonDetailResponse>(
    () => (isEditMode ? `admin-tool-comparisons-3?id=${id}` : null)
  );

  const [formData, setFormData] = useState<GroupComparisonFormData>(EMPTY_FORM);
  const [saveError, setSaveError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const { mutate: createGroupComparison } = useAdminMutation<{ ok: boolean; data: { id: string } }, unknown>(
    'admin-tool-comparisons-3',
    'POST'
  );
  const { mutate: updateGroupComparison } = useAdminMutation<{ ok: boolean }, unknown>(
    `admin-tool-comparisons-3?id=${id}`,
    'PUT'
  );

  useEffect(() => {
    document.title = isEditMode ? 'Edit 3-Tool Comparison | Admin' : 'New 3-Tool Comparison | Admin';
  }, [isEditMode]);
  ...
  async function handleSave() {
    ...
    if (uniqueToolIds.size !== filledMembers.length) {
      setSaveError('Each tool can only appear once in a comparison.');
      return;
    }
    ...
    setSaving(true);
    const result = isEditMode ? await updateGroupComparison(payload) : await createGroupComparison(payload);
    setSaving(false);
    if (!result.ok) {
      setSaveError(result.error?.message || 'Failed to save comparison');
      return;
    }
    navigate('/wp-admin/tool-comparisons-3');
  }

  if (isEditMode && isLoading) {
    return (
      <WpAdminLayout title="Edit 3-Tool Comparison">
  ...
  return (
    <WpAdminLayout title={isEditMode ? 'Edit 3-Tool Comparison' : 'New 3-Tool Comparison'}>
      <div className="max-w-2xl mx-auto">
        <Link to="/wp-admin/tool-comparisons-3" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 mb-4">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800 mb-4">
          Only publish a comparison once real editorial content exists for it in
          <code className="mx-1 px-1 py-0.5 bg-amber-100 rounded text-xs">src/data/groupComparisonContent/</code>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData((p) => ({ ...p, status: e.target.value as GroupComparisonFormData['status'] }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
