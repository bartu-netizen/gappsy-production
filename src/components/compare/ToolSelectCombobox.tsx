  ...
  return (
    <div ref={containerRef} className="relative flex-1 min-w-0">
      ...
      <button
        ...
        } ${open ? 'border-[#8B90D9] ring-2 ring-[#E0E3FC]' : ''}`}
      >
        {value ? (
          <>
            {value.logo ? (
            ...
            {!disabled && (
              <span
                ...
                className="text-slate-400 hover:text-slate-600 shrink-0 rounded focus:outline-none focus:ring-2 focus:ring-[#A8AEF0]"
              >
                <X className="w-4 h-4" />
              </span>
            )}
        ...
        <div className="absolute z-20 top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-[0_16px_40px_rgba(15,23,42,0.14)] overflow-hidden">
          <div className="flex items-center gap-2 px-3.5 h-11 border-b border-slate-100">
            ...
            <input
              ...
              autoComplete="off"
            />
            {loading && <Loader2 className="w-3.5 h-3.5 text-[#8B90D9] animate-spin shrink-0" aria-hidden="true" />}
          </div>
              ...
              <li key={tool.slug} role="presentation">
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-left transition-colors ${
                    index === activeIndex ? 'bg-[#EEF0FE]' : 'hover:bg-slate-50'
                  }`}
                >
                  {tool.logo ? (
