...
interface RoundupCardTool {
  slug: string;
  name: string;
  logo: string | null;
}

function ToolAvatar({ tool, index }: { tool: RoundupCardTool; index: number }) {
  return tool.logo ? (
    <img
      src={tool.logo}
      alt={tool.name}
...
export default function RoundupCard({ slug, title, tools }: { slug: string; title: string; tools: RoundupCardTool[] }) {
  return (
    <Link
      to={`/roundup/${slug}`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-400 mt-0.5">{tools.length}-tool roundup</p>
      </div>
