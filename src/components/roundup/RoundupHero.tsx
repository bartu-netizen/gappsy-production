...
function RoundupToolCard({ tool, bestFor }: { tool: CompareToolFacts; bestFor?: string }) {
...
export default function RoundupHero({ title, tools, bestFor }: { title: string; tools: CompareToolFacts[]; bestFor: Record<string, string> }) {
  return (
    <section className="max-w-5xl mx-auto px-4 sm:px-6 pt-4 pb-8 relative overflow-hidden">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {tools.map((tool) => (
          <RoundupToolCard key={tool.slug} tool={tool} bestFor={bestFor[tool.slug]} />
        ))}
      </div>
    </section>
  );
