  ...
  if (size === 'compact') {
    return (
      <Link
        ...
        className="group flex items-center gap-3 bg-white border border-[#eef0f3] rounded-xl px-4 py-3.5 transition-colors hover:border-[#C7CEE3] hover:bg-[#EEF1F7]/30"
      >
        <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center shrink-0">
          <FolderTree className="w-4 h-4 text-slate-400" />
        </div>
        <span className="flex-1 min-w-0 text-sm font-medium text-[#0B1221] truncate group-hover:text-[#0A1735] transition-colors">
          {category.name}
        </span>
  ...
  return (
    ...
    >
      <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 shadow-sm flex items-center justify-center mb-5">
        <FolderTree className="w-4.5 h-4.5 text-[#0A1735]" />
      </div>
      ...
      <div className="mt-auto flex items-center justify-between pt-1">
        ...
        <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-[#0A1735] group-hover:translate-x-0.5 transition-all" />
      </div>
    </Link>
  );
}
