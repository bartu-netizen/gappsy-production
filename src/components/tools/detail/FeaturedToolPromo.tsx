...
function WantYourProductHereLink({ className = '' }: { className?: string }) {
  return (
    <Link
      ...
      className={`flex items-center gap-1 text-[11px] font-semibold text-[#2952CC] hover:text-[#1F3FA3] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0A1735] rounded-sm ${className}`}
    >
...
export function FeaturedToolSidebarCard({ tool }: { tool: FeaturedTool }) {
  return (
    <div className="rounded-2xl bg-gradient-to-br from-[#EEF1F7] to-purple-50 border border-[#DEE3EF] p-4">
      <div className="flex items-center justify-between mb-2.5">
        <FeaturedBadge large />
      </div>
      <Link to={`/tools/${tool.slug}`} className="group flex items-start gap-3 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0A1735]">
        {tool.logo ? (
        ...
        <div className="min-w-0">
          <p className="font-bold text-[#0B1221] text-sm group-hover:text-[#0A1735] transition-colors">{tool.name}</p>
          {tool.short_description && <p className="text-xs text-slate-600 leading-relaxed line-clamp-2 mt-0.5">{tool.short_description}</p>}
        </div>
      </Link>
      <WantYourProductHereLink className="mt-2.5 pt-2.5 border-t border-[#DEE3EF]/80" />
    </div>
  );
}
...
export function FeaturedToolSidebarCompact({ tool }: { tool: FeaturedTool }) {
  return (
    <div className="rounded-xl bg-gradient-to-br from-[#EEF1F7]/70 to-purple-50/70 border border-[#DEE3EF] px-3 py-2.5">
      <Link
        to={`/tools/${tool.slug}`}
        className="group flex items-center gap-2.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0A1735] rounded-lg"
      >
        {tool.logo ? (
        ...
        <div className="min-w-0 flex-1">
          <FeaturedBadge />
          <p className="font-semibold text-[#0B1221] text-[13px] leading-tight truncate group-hover:text-[#0A1735] transition-colors">{tool.name}</p>
        </div>
        <ArrowRight className="w-3.5 h-3.5 text-slate-400 shrink-0" aria-hidden="true" />
      </Link>
      <WantYourProductHereLink className="mt-2 pt-2 border-t border-[#DEE3EF]/80" />
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-[#EEF1F7] to-purple-50 border border-[#DEE3EF] p-4">
      <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center mb-2.5 shadow-sm">
        <Rocket className="w-4.5 h-4.5 text-[#2952CC]" aria-hidden="true" />
      </div>
      ...
      <Link
        to={onboardingHref}
        className="flex items-center justify-center gap-1.5 w-full mt-3 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#0A1735] hover:bg-[#132952] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0A1735] focus-visible:ring-offset-2"
      >
...
export function FeaturedToolInlineCard({ tool }: { tool: FeaturedTool }) {
  return (
    <div className="not-prose rounded-2xl border border-[#DEE3EF] bg-gradient-to-br from-[#EEF1F7]/60 to-purple-50/40 p-5">
      ...
      <Link
        to={`/tools/${tool.slug}`}
        className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-[#2952CC] hover:bg-[#1F3FA3] transition-colors shrink-0 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0A1735] focus-visible:ring-offset-2"
      >
        View listing
        <ArrowRight className="w-3.5 h-3.5" aria-hidden="true" />
      </Link>
      </div>
      <WantYourProductHereLink className="mt-3 pt-3 border-t border-[#DEE3EF]/80" />
    </div>
  );
}
