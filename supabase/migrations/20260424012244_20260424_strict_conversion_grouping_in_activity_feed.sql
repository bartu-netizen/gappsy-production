/*
  # Strict conversion grouping in admin activity feed

  Reclassifies events so that the "conversion" group ONLY includes real
  revenue events (completed payments / paid activations). High-intent
  signals (checkout opened, activate clicked, availability submitted)
  now map to a dedicated "high_intent" group, not "conversion".

  1. Updates
    - `get_admin_activity_feed` event_group CASE:
      - conversion = checkout_completed, payment_succeeded, stripe_payment_succeeded,
        listing_activated_paid, top25_purchase_completed, spotlight_purchase_completed
      - high_intent = checkout_opened, your_agency_activate_clicked, availability_submitted
      - engagement = replay engaged, ledger viewed, profile clicked, demo submitted, etc.
      - negative = manage_remove_started, manage_remove_confirmed
      - view = everything else

  2. Notes
    1. Intent weights unchanged — grouping only.
    2. No changes to tracking, logging, funnels, or event names.
*/
...
GRANT EXECUTE ON FUNCTION public.get_admin_activity_feed(integer, text, uuid) TO service_role;
