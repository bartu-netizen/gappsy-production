/*
  # Fix compute_agency_intent_scores: complete all_combos UNION

  ## Problem
  The all_combos CTE only unioned fe_agg, click_agg, and yafe_agg.
  An agency that only has repeat-session (session_agg) or dwell-time
  (eng_agg) signals would be silently dropped from the final result
  because it never appeared in all_combos.

  ## Fix
  Add session_agg and eng_agg to the UNION so every signal source
  is guaranteed to be represented regardless of which other tables
  have data for the agency.

  ## No other changes — scoring weights, recency multipliers,
  deepest_step ladder, and SECURITY DEFINER are all preserved.
*/
...
AS $$
  ...
  /* ─── 6. ALL COMBOS — now includes ALL five signal sources ─────── */
  all_combos AS (
    ...
    UNION
    SELECT agency_id, sl FROM session_agg   /* FIX: was missing */
    UNION
    SELECT agency_id, sl FROM eng_agg       /* FIX: was missing */
  ),
