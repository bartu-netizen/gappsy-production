/*
  # Fix get_other_agencies_stats: cast stripe_orders.agency_id to uuid

  ## Root Cause
  The previous migration added a stripe_orders EXISTS check but compared
  so.agency_id (TEXT) directly with oa.id (UUID), causing:
    ERROR: operator does not exist: text = uuid

  This broke the entire RPC — it threw on every call, the edge function swallowed
  the error and returned {}, and all summary cards showed zero.

  ## Fix
  Cast so.agency_id to uuid for the comparison: so.agency_id::uuid = oa.id
*/
