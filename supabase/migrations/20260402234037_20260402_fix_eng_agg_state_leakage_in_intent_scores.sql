/*
  # Fix compute_agency_intent_scores: remove eng_agg null-state leakage

  ## Problem
  The eng_agg CTE in compute_agency_intent_scores contained a state filter
  with a third OR arm:

    AND (
      p_state_slug IS NULL
      OR COALESCE(fs.state_slug, '') = p_state_slug
      OR fs.state_slug IS NULL          -- ← LEAKS
    )

  When p_state_slug = 'oregon', any agency_funnel_engagement row whose
  funnel_session has state_slug IS NULL passes this filter.
  Those rows land in eng_agg with sl = '' (COALESCE(NULL,'')).
  They propagate through all_combos → combined → scored.
  The final RPC result contains rows with state_slug = '' for agencies
  that actually belong to new-jersey, north-carolina, new-york, etc.

  loadIntentScores() added those IDs to the intent score map.
  prefetchHighIntentAgencies() fetched them without a state guard.
  Result: wrong-state agencies appeared in Oregon's Top 12 showcase.

  ## Fix
  Remove `OR fs.state_slug IS NULL` from eng_agg.

  When p_state_slug IS NOT NULL (state-scoped call):
    - COALESCE(NULL, '') = '' which is NOT equal to 'oregon'
    - null-state sessions are correctly excluded
  When p_state_slug IS NULL (global/admin call):
    - The first arm (p_state_slug IS NULL) is TRUE for all rows
    - null-state sessions are still included (correct for global queries)

  ## No other changes
  All scoring weights, recency multipliers, deepest_step ladder,
  all_combos UNION, token_visit_score column name, SECURITY DEFINER,
  and GRANT are preserved exactly from the previous migration.
*/
