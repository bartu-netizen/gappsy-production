/*
  # Fix get_other_agencies_stats: cast agency_id text → uuid

  agency_listings.agency_id is stored as TEXT while other_agencies.id is UUID.
  The comparison al.agency_id = oa.id raises "operator does not exist: text = uuid".
  Fix by casting al.agency_id to uuid in the EXISTS subquery.
*/
    ...
    WHERE oa.state_slug = p_state_slug
      AND (
        ...
        OR EXISTS (
          ...
          WHERE oae.agency_id                  = oa.id::text
