/*
  # Hardened admin activity feed v5

  Rebuilds get_admin_activity_feed with:
  - Deterministic identity fallback chain (id → slug+state → slug → norm_name+state → norm_name → fuzzy trigram)
  - Returns identity_confidence (high/medium/low/none) and identity_method
  - Reweighted intent scoring (checkout_opened=25, CTA=15, availability=12, view_requests=8, ledger=6)
  - intent_stage mapping (discovery/interest/evaluation/high_intent/purchase_ready/converted/churn_risk)
  - Explicit column aliases in first UNION branch (fixes prior 42703)
  - Bounded via row-windowing to keep work within statement timeout
*/
...
AS $$
  WITH raw AS (
    ...
    SELECT
      ...
      s.state_slug,
      s.agency_name,
      COALESCE(s.status::text, 'stripe_order'),
      'payment'::text,
      'stripe'::text,
      s.metadata
  ...
  windowed AS (
    ...
    LIMIT LEAST(GREATEST(p_limit, 1) * 4, 2000)
  ),
  prepared AS (
    SELECT
      w.*,
      NULLIF(
        COALESCE(
          w.src_agency_name,
          w.metadata->>'agency_name',
          w.metadata->>'agencyName',
          w.metadata->>'name'
        ),
        ''
      ) AS meta_name,
      NULLIF(
        COALESCE(
          w.agency_slug,
          w.metadata->>'agency_slug',
          w.metadata->>'agencySlug',
          w.metadata->>'slug'
        ),
        ''
      ) AS eff_slug,
  ...
  resolved AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE p.agency_id IS NOT NULL AND oa.id::text = p.agency_id
      LIMIT 1
    ) m ON TRUE
    LEFT JOIN LATERAL (
      -- 2. slug + state
      ...
      WHERE m.matched_id IS NULL
        AND p.eff_slug IS NOT NULL AND p.state_slug IS NOT NULL
        AND oa.slug = p.eff_slug AND oa.state_slug = p.state_slug
      LIMIT 1
    ) m2(matched_id, matched_name, matched_slug, method, confidence)
      ON m.matched_id IS NULL
    LEFT JOIN LATERAL (
      -- 3. slug only
    ...
    LEFT JOIN LATERAL (
      -- 6. fuzzy trigram match (uses GIN index on name_norm)
  ...
  enriched AS (
    SELECT
      ...
      r.funnel_step,
      r.source,
      COALESCE(r.matched_id, r.agency_id) AS agency_id,
      ...
      COALESCE(r.confidence, CASE WHEN r.meta_name IS NOT NULL OR r.eff_slug IS NOT NULL THEN 'low' ELSE 'none' END) AS identity_confidence,
      CASE
      ...
      jsonb_build_object(
        ...
        'eff_slug', r.eff_slug,
        'matched_via', r.method
      ) AS identity_debug
    FROM resolved r
  )
  SELECT
    e.id, e.created_at, e.event_name, e.funnel_step, e.source,
