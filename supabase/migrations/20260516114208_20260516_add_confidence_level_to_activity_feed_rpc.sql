/*
  # Add confidence_level to admin activity feed RPC

  ## Summary
  Updates the `get_admin_activity_feed` function to expose the actual confidence
  classification (`intent_level`) from source tables as `confidence_level` in
  the output. This enables proper traffic mode filtering (All / Likely Human /
  Verified Human) in the admin activity feed.

  ## Changes
  - Adds `confidence_level` output column to `get_admin_activity_feed`
  - Sources it from `funnel_events.intent_level` and `manage_funnel_events.intent_level`
  - Other sources (your_agency, availability_requests, stripe_orders) return NULL
    indicating "unclassified" confidence

  ## Security
  No RLS changes. Function remains SECURITY DEFINER with restricted search_path.
*/
...
RETURNS TABLE (
  ...
  agency_id text, agency_name text, agency_slug text,
  state_slug text, state_name text,
  event_name text, event_group text, funnel_stage text, funnel_step text,
  source text, intent_weight integer, readable_label text, metadata jsonb,
  intent_stage text, identity_method text, identity_confidence text,
...
AS $$
  WITH raw AS (
    SELECT
      ...
      f.intent_level AS confidence_level
  ...
  SELECT
    ...
    COALESCE(r.state_slug, r.matched_state)                    AS state_slug,
    NULL::text                                                 AS state_name,
    r.event_name,
    CASE
      WHEN r.event_name IN ('availability_submitted','availability_request_submitted') THEN 'high_intent'
      WHEN r.event_name IN ('stripe_checkout_opened','checkout_opened','stripe_checkout_started') THEN 'checkout'
      WHEN r.event_name ILIKE '%cta%click%' OR r.event_name ILIKE '%click_cta%' OR r.event_name = 'cta_clicked' THEN 'high_intent'
      WHEN r.event_name IN ('view_requests_clicked','view_matches_clicked','your_agency_view_requests_clicked') THEN 'high_intent'
      WHEN r.source = 'stripe' AND r.event_name IN ('completed','paid') THEN 'conversion'
      WHEN r.event_name ILIKE '%activation%' OR r.event_name ILIKE '%listing_activated%' THEN 'conversion'
      WHEN r.source = 'manage_funnel' AND r.event_name ILIKE '%remove%' THEN 'negative'
      WHEN r.event_name ILIKE '%ledger%' OR r.event_name ILIKE '%missed%opportun%' THEN 'engagement'
      WHEN r.event_name ILIKE '%profile%view%' OR r.event_name ILIKE '%landing%' OR r.event_name ILIKE '%page_view%' THEN 'view'
      ELSE 'engagement'
    END                                                        AS event_group,
    CASE
      WHEN r.event_name IN ('availability_submitted','availability_request_submitted') THEN 'high_intent'
      ...
      WHEN r.source = 'manage_funnel' AND r.event_name ILIKE '%remove%' THEN 'negative'
      WHEN r.event_name ILIKE '%ledger%' OR r.event_name ILIKE '%missed%opportun%' THEN 'engagement'
      WHEN r.event_name ILIKE '%profile%view%' OR r.event_name ILIKE '%landing%' OR r.event_name ILIKE '%page_view%' THEN 'awareness'
      ELSE 'engagement'
    END                                                        AS funnel_stage,
    r.funnel_step,
    r.source,
    (CASE
      ...
      WHEN r.event_name IN ('view_requests_clicked','view_matches_clicked','your_agency_view_requests_clicked') THEN 8
      ...
      WHEN r.source = 'manage_funnel' AND r.event_name ILIKE '%remove%' THEN -10
      ELSE 1
    END)::integer                                              AS intent_weight,
    NULL::text                                                 AS readable_label,
    r.metadata,
    CASE
  ...
  FROM resolved r
  WHERE (p_agency_id IS NULL OR COALESCE(r.matched_id, r.eff_agency_id) = p_agency_id::text)
