/*
  # Extend agency intent scoring with activation + standard upgrade + top25 confirm events

  1. Context
    - The frontend now emits canonical funnel events for the activation /
      standard-upgrade-offer / top25-confirm / checkout flow:
        * activation_page_viewed
        * activation_option_clicked (dedup by plan + position)
        * standard_upgrade_offer_viewed
        * standard_upgrade_offer_upgrade_clicked (dedup by position)
        * standard_upgrade_offer_standard_continued
        * top25_confirm_viewed
        * top25_confirm_addon_toggled (only scored when action = 'enabled')
        * top25_confirm_continue_clicked
    - STRIPE_CHECKOUT_OPENED / checkout_page_viewed already scored by
      checkout_bonus (+20 first + up to +24 repeats); not re-weighted here.

  2. Changes
    - Replace compute_agency_intent_scores(text) with a version that adds new
      weights. Dedup-aware events use DISTINCT counts on metadata keys so that
      duplicate clicks from React strict mode / double-taps contribute at most
      one unit of score per unique combo. Non-dedup events use the historical
      SUM(CASE...) pattern.

  3. Weights added
    - activation_page_viewed                    => +10
    - activation_option_clicked                 => +25 per distinct (plan, position)
    - standard_upgrade_offer_viewed             => +15
    - standard_upgrade_offer_upgrade_clicked    => +35 per distinct position
    - standard_upgrade_offer_standard_continued => +20
    - top25_confirm_viewed                      => +20
    - top25_confirm_addon_toggled               => +10 per distinct addon (action=enabled only)
    - top25_confirm_continue_clicked            => +35

  4. Security
    - SECURITY DEFINER retained, search_path pinned to public.
    - EXECUTE grant for anon and authenticated preserved.
*/
...
AS $$
    ...
    SELECT
      ...
      SUM(
        CASE fe.event_name
          ...
          WHEN 'your_agency_view_requests_clicked'                  THEN 4
          WHEN 'owner_view_requests_clicked'                        THEN 4
          WHEN 'your_agency_ledger_page_viewed'                     THEN 6
          WHEN 'owner_ledger_view_opened'                           THEN 6
          WHEN 'your_agency_requests_visible'                       THEN 8
          WHEN 'owner_ledger_engaged'                               THEN 8
          WHEN 'your_agency_activate_clicked'                       THEN 12
          ...
          WHEN 'activation_page_viewed'                             THEN 10
          WHEN 'standard_upgrade_offer_viewed'                      THEN 15
          WHEN 'standard_upgrade_offer_standard_continued'          THEN 20
          WHEN 'top25_confirm_viewed'                               THEN 20
          WHEN 'top25_confirm_continue_clicked'                     THEN 35
          ELSE 0
        END
      )                                          AS base_event_score
  ...
  -- Dedup-aware: +25 per DISTINCT (plan, position) combo for
  -- activation_option_clicked. Max 5 unique combos scored.
  activation_option_agg AS (
    SELECT
      fs.agency_id,
      COALESCE(fe.state_slug, fs.state_slug)                    AS sl,
      (LEAST(COUNT(DISTINCT COALESCE(fe.metadata->>'plan','')
                          || '|' ||
                          COALESCE(fe.metadata->>'position','')), 5) * 25)::int
                                                                AS activation_option_score
    FROM funnel_events fe
    JOIN funnel_sessions fs ON fs.id = fe.session_id
    WHERE fe.funnel_name = 'your_agency'
      AND fe.event_name = 'activation_option_clicked'
      AND fs.agency_id IS NOT NULL
      AND fs.agency_id <> ''
      AND (
        p_state_slug IS NULL
    ...
    GROUP BY fs.agency_id, COALESCE(fe.state_slug, fs.state_slug)
  ),

  -- Dedup-aware: +35 per DISTINCT position for
  -- standard_upgrade_offer_upgrade_clicked. Max 3 unique positions scored.
  std_upgrade_click_agg AS (
    SELECT
      fs.agency_id,
      COALESCE(fe.state_slug, fs.state_slug)                    AS sl,
      (LEAST(COUNT(DISTINCT COALESCE(fe.metadata->>'position','')), 3) * 35)::int
                                                                AS std_upgrade_click_score
    FROM funnel_events fe
    JOIN funnel_sessions fs ON fs.id = fe.session_id
    WHERE fe.funnel_name = 'your_agency'
      AND fe.event_name = 'standard_upgrade_offer_upgrade_clicked'
      AND fs.agency_id IS NOT NULL
      AND fs.agency_id <> ''
      AND (
        p_state_slug IS NULL
  ...
  -- Dedup-aware + filtered: +10 per DISTINCT addon for
  -- top25_confirm_addon_toggled, only when action='enabled'. Max 2 addons.
  addon_toggle_agg AS (
    SELECT
      fs.agency_id,
      COALESCE(fe.state_slug, fs.state_slug)                    AS sl,
      (LEAST(COUNT(DISTINCT COALESCE(fe.metadata->>'addon','')), 2) * 10)::int
                                                                AS addon_toggle_score
    FROM funnel_events fe
    JOIN funnel_sessions fs ON fs.id = fe.session_id
    WHERE fe.funnel_name = 'your_agency'
      AND fe.event_name = 'top25_confirm_addon_toggled'
      AND fe.metadata->>'action' = 'enabled'
      AND fs.agency_id IS NOT NULL
      AND fs.agency_id <> ''
      AND (
        p_state_slug IS NULL
  ...
  all_combos AS (
    ...
    UNION
    SELECT agency_id, sl FROM activation_option_agg
    UNION
    SELECT agency_id, sl FROM std_upgrade_click_agg
    UNION
    SELECT agency_id, sl FROM addon_toggle_agg
  ),
  ...
  combined AS (
    SELECT
      ...
      COALESCE(ao.activation_option_score, 0)::int                  AS activation_option_score,
      COALESCE(su.std_upgrade_click_score, 0)::int                  AS std_upgrade_click_score,
      COALESCE(at.addon_toggle_score, 0)::int                       AS addon_toggle_score,
    ...
    LEFT JOIN activation_option_agg ao ON ao.agency_id = ac.agency_id AND ao.sl = ac.sl
    LEFT JOIN std_upgrade_click_agg su ON su.agency_id = ac.agency_id AND su.sl = ac.sl
    LEFT JOIN addon_toggle_agg at ON at.agency_id = ac.agency_id AND at.sl = ac.sl
  ),
  ...
  scored AS (
    SELECT
      ...
      (c.event_score + c.checkout_score + c.token_visit_score
       ...
       + c.activation_option_score + c.std_upgrade_click_score
       + c.addon_toggle_score
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'activation_option_score',   s.activation_option_score,
      'std_upgrade_click_score',   s.std_upgrade_click_score,
      'addon_toggle_score',        s.addon_toggle_score,
...
COMMENT ON FUNCTION compute_agency_intent_scores(text) IS
  'Multi-source agency intent scoring. Adds activation/standard-upgrade/top25-confirm '
  'event weights: activation_page_viewed +10, activation_option_clicked +25 per '
  'distinct (plan,position) up to 5, standard_upgrade_offer_viewed +15, '
  'standard_upgrade_offer_upgrade_clicked +35 per distinct position up to 3, '
  'standard_upgrade_offer_standard_continued +20, top25_confirm_viewed +20, '
  'top25_confirm_addon_toggled +10 per distinct addon (action=enabled only, up to 2), '
  'top25_confirm_continue_clicked +35. checkout_opened continues to be scored '
  'via checkout_bonus (not duplicated).';
