/*
  # Fix get_other_agencies_stats: authoritative paid logic

  ## Problem
  The previous version of paid_status CTE only checked agency_listings.paid_active,
  missing the Stripe-confirmed path (is_paid_listing=true AND stripe_orders.payment_status='paid').
  This caused agencies like "Studio Forged" (is_paid_listing=true, no stripe_orders row)
  to be incorrectly counted as paid via agency_listings.paid_active when the webhook
  sets both fields simultaneously.

  ## New authoritative paid logic (matches client effectivePaid()):
  1. Manual override: paid_override_active=true AND paid_override_value=true/false
  2. Stripe confirmed: is_paid_listing=true AND EXISTS stripe_orders.payment_status='paid'
  3. Activated listing: agency_listings.paid_active=true AND listing_tier='standard'

  An agency is paid only if at least one of the above is true.
  is_paid_listing alone (without a stripe_orders row) does NOT count as paid.
*/
