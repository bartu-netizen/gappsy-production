/*
  # Activity Feed — Paid-Only Conversion Hardening

  1. Changes
    - Remove ambiguous `listing_activated` from the conversion whitelist.
      Only `listing_activated_paid` counts as conversion.
    - Stripe rows now carry `payment_status` and `paid_at` into metadata and
      are gated on `payment_status='paid' OR paid_at IS NOT NULL` before
      being counted as conversion.
    - Ambiguous stripe statuses like `active` (which can mean subscription
      active without payment yet) no longer count unless the paid guard passes.
...
WITH raw AS (
  ...
  SELECT s.id::text, s.created_at, s.agency_id, NULL::text, s.state_slug, s.agency_name,
         COALESCE(s.status::text,'stripe_order'), 'payment'::text, 'stripe'::text,
         COALESCE(s.metadata,'{}'::jsonb)
           || jsonb_build_object(
                'payment_status', s.payment_status,
                'paid_at', s.paid_at,
                'is_paid_order', (s.payment_status = 'paid' OR s.paid_at IS NOT NULL)
              )
...
classified AS (
  SELECT r.*,
    -- STRICT paid-only conversion whitelist:
    CASE
      -- Stripe rows: require paid guard (payment_status='paid' OR paid_at IS NOT NULL)
      WHEN r.source = 'stripe'
           AND COALESCE((r.metadata->>'is_paid_order')::boolean, false) = true
           AND lower(r.event_name) IN ('completed','paid','succeeded','listing_activated')
        THEN TRUE
      -- Non-stripe: only strict paid event names (exclude bare 'listing_activated')
...
SELECT
  ...
  CASE
    ...
    WHEN c.source='stripe' AND NOT COALESCE((c.metadata->>'is_paid_order')::boolean,false)
      THEN 'Stripe order — ' || initcap(replace(c.event_name,'_',' ')) || ' (unpaid)'
