/*
  # Align intent scoring with canonical your_agency_* event names

  1. Context
    - The frontend now emits canonical event names on the /your-agency funnel:
      - your_agency_view_requests_clicked       (was owner_view_requests_clicked)
      - your_agency_ledger_page_viewed          (was owner_ledger_view_opened)
      - your_agency_requests_visible            (was owner_ledger_engaged)
      - your_agency_activate_clicked            (was owner_ledger_activation_clicked)
    - Intent scoring previously only recognized the legacy owner_* names, so
      new events were being ignored by compute_agency_intent_scores(...).

  2. Changes
    - Replace compute_agency_intent_scores(text) with an updated version that
      scores BOTH the legacy owner_* names and the new canonical names with
      identical weights. Historical rows (pre-rename) keep scoring correctly,
      and new rows score consistently going forward.

  3. Scoring weights (unchanged values, new aliases added)
    - your_agency_view_requests_clicked   => +4 (parity with owner_view_requests_clicked)
    - your_agency_ledger_page_viewed      => +6 (parity with owner_ledger_view_opened)
    - your_agency_requests_visible        => +8 (parity with owner_ledger_engaged)
    - your_agency_activate_clicked        => +12 (parity with owner_ledger_activation_clicked)
...
COMMENT ON FUNCTION compute_agency_intent_scores(text) IS
  ...
  '(canonical: your_agency_view_requests_clicked +4, your_agency_ledger_page_viewed +6, '
  'your_agency_requests_visible +8, your_agency_activate_clicked +12; legacy owner_* '
  'aliases preserved for historical rows), token visits, profile clicks, repeat '
  'sessions, dwell time, request_replay funnel signals, and replay repeat bonus '
  '(2+→+5, 3+→+10, 5+→+15). replay_score capped at 60.';
