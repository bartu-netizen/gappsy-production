/*
  # Relax funnel_events identity filter in get_admin_activity_feed

  ## Problem
  The funnel_events union previously required `f.agency_id IS NOT NULL`, which
  dropped rows whose agency identity lived in metadata (agency_id / agency_slug /
  agency_name). The downstream identity fallback chain never got a chance to
  resolve these rows.

  ## Change
  Replace the WHERE clause with one that keeps a row if ANY identity token is
  present — either the direct column or any of the metadata key variants. All
  other logic, return types, and intent scoring are untouched.

  1. Identity tokens checked
     - f.agency_id
     - metadata: agency_id / agencyId / aid
     - metadata: agency_slug / agencySlug / slug
     - metadata: agency_name / agencyName / company_name / companyName
*/

CREATE OR REPLACE FUNCTION public.get_admin_activity_feed(p_limit integer DEFAULT 200, p_state_slug text DEFAULT NULL::text, p_agency_id uuid DEFAULT NULL::uuid, p_window_minutes integer DEFAULT NULL::integer)
 RETURNS TABLE(id text, created_at timestamp with time zone, agency_id text, agency_name text, agency_slug text, state_slug text, state_name text, event_name text, event_group text, funnel_stage text, funnel_step text, source text, intent_weight integer, readable_label text, identity_source text, identity_debug jsonb, metadata jsonb)
 LANGUAGE sql
 SECURITY DEFINER
...
AND (
  ...
  OR f.metadata ? 'agency_id'
  OR f.metadata ? 'agencyId'
  OR f.metadata ? 'aid'
  OR f.metadata ? 'agency_slug'
  OR f.metadata ? 'agencySlug'
  OR f.metadata ? 'slug'
  OR f.metadata ? 'agency_name'
  OR f.metadata ? 'agencyName'
  OR f.metadata ? 'company_name'
  OR f.metadata ? 'companyName'
)
