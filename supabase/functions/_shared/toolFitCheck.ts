...
// Finds a genuinely relevant, already-Featured (paying) alternative for an
...
// and nothing else), so this requires 2+ real shared tags, and returns null
// rather than a weak same-category guess when a tool has no tags of its
// own to check against. Restricted to featured=true specifically, since
// that's the "paid Gappsy for visibility" concept already disclosed
// throughout ask-gappsy's system prompts — never surfaced as an unbiased
// pick without that label.
export async function findFeaturedAlternative(
  ...
  const { data: links } = await supabase
    .from("tool_category_links")
    .select("tools!inner(id, slug, name, logo, short_description, rating, review_count, is_open_source, status, featured)")
    // deno-lint-ignore no-explicit-any
