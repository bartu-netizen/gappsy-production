...
async function resolveCanonicalMembers(supabase: any, members: MemberInput[]) {
  ...
  if (ids.length < MIN_MEMBERS) {
    throw new ValidationError(`A roundup needs at least ${MIN_MEMBERS} distinct tools`);
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function attachMembers(supabase: any, roundupIds: string[]) {
  if (roundupIds.length === 0) return new Map();
  const { data, error } = await supabase
    .from("tool_roundup_members")
    .select("roundup_id, sort_order, best_for, tools(id, slug, name, logo, status)")
    .in("roundup_id", roundupIds)
    .order("sort_order", { ascending: true });
  if (error) throw new Error(`Failed to load roundup members: ${error.message}`);
  const map = new Map<string, unknown[]>();
  for (const row of data || []) {
    const list = map.get(row.roundup_id) || [];
    list.push({ sort_order: row.sort_order, best_for: row.best_for, tool: row.tools });
    map.set(row.roundup_id, list);
  }
  return map;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function replaceMembers(supabase: any, roundupId: string, members: MemberInput[]) {
  ...
  const { error: deleteError } = await supabase.from("tool_roundup_members").delete().eq("roundup_id", roundupId);
  ...
  const { error: insertError } = await supabase.from("tool_roundup_members").insert(rows);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      if (id) {
        const { data: roundup, error } = await supabase.from("tool_roundups").select("*").eq("id", id).maybeSingle();
        if (error) return jsonResponse({ ok: false, error: error.message }, 500);
        if (!roundup) return jsonResponse({ ok: false, error: "Roundup not found" }, 404);

        const membersMap = await attachMembers(supabase, [id]);
        return jsonResponse({ ok: true, data: { ...roundup, members: membersMap.get(id) || [] } });
      }

      const { data: roundups, error } = await supabase.from("tool_roundups").select("*").order("created_at", { ascending: false });
      if (error) return jsonResponse({ ok: false, error: error.message }, 500);

      const membersMap = await attachMembers(supabase, (roundups || []).map((r: { id: string }) => r.id));
      const data = (roundups || []).map((r: { id: string }) => ({ ...r, members: membersMap.get(r.id) || [] }));
    ...
    if (req.method === "POST") {
      ...
      const { data: existing } = await supabase.from("tool_roundups").select("id").eq("slug", slug).maybeSingle();
      if (existing) {
        return jsonResponse({ ok: false, error: `A roundup for this tool set already exists ("${slug}")` }, 409);
      }

      const { data: newRoundup, error: insertError } = await supabase
        .from("tool_roundups")
        .insert({
          slug,
      ...
      try {
        await replaceMembers(supabase, newRoundup.id, payload.members);
      } catch (memberError) {
        // Roll back the orphaned parent row so a failed member write never
        // leaves a slug-only roundup with no tools behind.
        await supabase.from("tool_roundups").delete().eq("id", newRoundup.id);
        const msg = memberError instanceof Error ? memberError.message : "Failed to save roundup members";
        return jsonResponse({ ok: false, error: msg }, 500);
      }

      return jsonResponse({ ok: true, data: newRoundup });
    }

    if (req.method === "PUT") {
      if (!id) return jsonResponse({ ok: false, error: "Roundup ID is required" }, 400);

      const { data: existing } = await supabase.from("tool_roundups").select("*").eq("id", id).maybeSingle();
      if (!existing) return jsonResponse({ ok: false, error: "Roundup not found" }, 404);
      ...
      if (Array.isArray(payload.members) && payload.members.length > 0) {
        ...
        const { data: slugExists } = await supabase.from("tool_roundups").select("id").eq("slug", slug).neq("id", id).maybeSingle();
        if (slugExists) {
          return jsonResponse({ ok: false, error: `A roundup for this tool set already exists ("${slug}")` }, 409);
        }
        updates.slug = slug;
      ...
      if (updateError) {
        if (updateError.code === UNIQUE_VIOLATION) {
          return jsonResponse({ ok: false, error: "A roundup for this tool set already exists" }, 409);
        }
    ...
    if (req.method === "DELETE") {
      ...
      const { error } = await supabase.from("tool_roundups").delete().eq("id", id);
      if (error) return jsonResponse({ ok: false, error: error.message }, 500);
